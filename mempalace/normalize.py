...
def _try_gemini_json(data: dict) -> Optional[str]:
    ...
    for msg in messages:
        ...
        if msg_type in ("user", "human"):
            transcript_messages.append(("user", text))
        elif msg_type in ("model", "assistant", "ai"):
            transcript_messages.append(("assistant", text))

    if not transcript_messages:
        return None

    return _messages_to_transcript(transcript_messages)
